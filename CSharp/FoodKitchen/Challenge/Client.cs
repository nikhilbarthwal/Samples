...
class Client(string endpoint, string auth)
{
    ...
    public async Task<Problem> NewProblemAsync(string name, long seed = 0)
    {
        ...
        Log.Info("Client", $"Fetched new test problem, id={id}: {url}");
        var orders = await response.Content.ReadFromJsonAsync<List<Order>>();
        return new Problem(id, orders ?? []);
    }

    private class Options(TimeSpan rate, TimeSpan min, TimeSpan max)
    {
    ...
    private class Solution(Options options, IReadOnlyList<Action> actions)
    {
        [JsonPropertyName("options")]
        public Options Options { get; init; } = options;

        [JsonPropertyName("actions")]
        public IReadOnlyList<Action> Actions { get; init; } = actions;
    }

    /// <summary>
    /// SolveAsync submits a sequence of actions and parameters as a solution to a test problem. Returns test result.
    /// </summary>
    public async Task<string> SolveAsync(string testId, TimeSpan rate, TimeSpan min, TimeSpan max,
